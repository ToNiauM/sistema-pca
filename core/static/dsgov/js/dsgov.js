...
(function () {
  ...
  function iniciar(raiz) {
    ...
    COMPONENTES.forEach(function (par) {
      ...
      /* O popover do calendário usa a mesma classe .br-tooltip mas tem
         comportamento próprio (ver bloco abaixo); excluído aqui para o
         BRTooltip nunca instanciar duas vezes o mesmo balão. */
  ...
  /* ---------- Atalho de busca: Alt+Shift+P ---------- */
  /* Listener único, registrado fora de iniciar() (nunca roda de novo a
     cada troca HTMX). Abre e focaliza a busca do header sem navegar;
     devolve o foco anterior ao fechar; respeita o modal de acompanhamento
     aberto. */
  ...
  /* Comportamento próprio, nunca a inicialização automática do BRTooltip:
     mostrar em hover/foco, fixar em clique/Enter/toque, Esc ou "Fechar
     detalhes" fecham e devolvem o foco ao ativador. Delegado em document,
     registrado uma única vez, fora de iniciar(). Nunca usa o atributo
     HTML5 nativo popover (a Popover API esconderia o elemento
     permanentemente sem chamar showPopover(), API que este comportamento
     não usa). */
  ...
  /* Esconder ao sair, exceto quando fixado ou quando o ponteiro entrou no
     próprio popover (conteúdo acessível ao mover o ponteiro para dentro
     dele). */
  ...
  /* ---------- aria-expanded do acordeão ---------- */
  /* O core.min.js só alterna o atributo active no .item, nunca
     aria-expanded do button.header — sem isto um leitor de tela nunca
     anuncia expandido/recolhido nos acordeões. Listener em document,
     registrado depois do clique já ter alternado active (o listener do
     vendor sempre roda antes de um delegado no mesmo evento). Sincroniza
     todos os botões do mesmo .br-accordion a cada clique. */
  document.addEventListener("click", function (ev) {
  ...
  /* ---------- Restauração de rolagem em <form method=get> sem HTMX ----------
     Telas que recarregam a página inteira a cada troca de filtro voltavam
     ao topo a cada submissão. /tabela (hx-get) já preserva rolagem via
     HTMX/hx-push-url e é excluída por guarda explícita abaixo.
     sessionStorage guarda só a posição de rolagem por pathname, nenhum
     dado pessoal. Restaura só quando document.referrer aponta para o
     mesmo pathname. */
