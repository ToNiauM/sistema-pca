#!/usr/bin/env python3
"""Verificador de conformidade com o DSGov (skill dsgov). BLOQUEANTE: saída 1 se houver erro.

Uso:
    python3 <skill>/scripts/verificar.py <raiz-do-projeto> [--json] [--avisos-como-erros]
...
CORE_CSS = SKILL / "assets/vendor/govbr-ds/3.7.0/core.min.css"
FA_CSS = SKILL / "assets/vendor/fontawesome/5.15.4/css/all.min.css"
SKILL_CSS_DIR = SKILL / "assets/projeto/core/static/dsgov/css"

# Fallback de portabilidade (ver .claude/skills/dsgov/VENDOR-NOTA.md): esta
# cópia da skill não traz `assets/vendor/` (~4 MB já vendorizados em
# `core/static/dsgov/vendor/` do próprio projeto). Se o caminho acima não
# existir, resolve o mesmo asset dentro da raiz do projeto verificado.
def _resolver_vendor(caminho_skill: Path, raiz: Path, relativo_projeto: str) -> Path:
    if caminho_skill.exists():
        return caminho_skill
    return raiz / relativo_projeto
...
def main():
    ...
    core_css = _resolver_vendor(
        CORE_CSS, raiz, "core/static/dsgov/vendor/govbr-ds/core.min.css"
    )
    fa_css = _resolver_vendor(
        FA_CSS, raiz, "core/static/dsgov/vendor/fontawesome/css/all.min.css"
    )
